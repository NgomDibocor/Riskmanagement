({	/**
	 ...
	 * @history 2018-08-24 : Salimata NGOM - Implementation
	 */
	sendMailContactWorkshop : function(component, row) {
		...
		// check if Email field is Empty or not contains @ so display a alert
		// message
		...
		action
				.setCallback(
						...
						function(response) {
							...
							if (state === "SUCCESS") {
								var storeResponse = response.getReturnValue();
								// if state of server response is comes
								// "SUCCESS",
								// display the success message box by set
								// mailStatus attribute to true
								component.set("v.email", getEmail);

								// get contact workshop
								var contactworkshop = component
										.get('c.getContactWorkshop');
								contactworkshop.setParams({
									"item" : component.get('v.workshop').Id,
									"contact" : row.Id
								});
												...
												function(respcontactworkshop) {
													...
													if (state == "SUCCESS") {
														if ($A.util
															...
															var contactworkshopItem = respcontactworkshop
																	.getReturnValue();
															// update field
															// orm_notification__c
																				...
																				if (state === "SUCCESS") {
																					component
																							.set(
																									...
																									true);
																					this
																							.refreshContactWorkshop(component);
																				} else if (state === "ERROR") {
																					let
																					errors = response
																							.getError();
